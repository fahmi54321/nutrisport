...
class AdminRepositoryImpl : AdminRepository {
    ...
    override fun readLastTenProducts(): Flow<RequestState<List<Product>>> = channelFlow {
        try {
            ...
            send(RequestState.Error("Error while reading the last 10 items from the database: $e"))
        }
    }
    ...
    override suspend fun readProductById(id: String): RequestState<Product> {
        return try {
            val userId = getCurrentUserId()
            if (userId != null) {
                val databse = Firebase.firestore
                val document = databse.collection(collectionPath = "product")
                    .document(id)
                    .get()
                if (document.exists) {
                    val product = Product(
    ...
    override suspend fun updateProduct(
        ...
        try {
            ...
            if (userId != null) {
                ...
                if (existingProduct.exists) {
                    productCollection.document(product.id)
                        .update(product.copy(title = product.title.uppercase()))
                    onSuccess()
                } else {
                    onError("Selected product not found.")
                }
    ...
    override suspend fun deleteProduct(
        ...
        try {
            ...
            onError("Error while deleting a product: ${e.message}")
        }
    }
        ...
        channelFlow {
            try {
                ...
                if (userId != null) {
                    ...
                    database.collection(collectionPath = "product")
                        .orderBy("title")
                        .startAtFieldValues { arrayOf<Any?>(queryText).forEach { add(it) } }
                        .endAtFieldValues { arrayOf<Any?>(endText).forEach { add(it) } }
                        .snapshots
                        .collectLatest { query ->
                            val products = query.documents.map { document ->
                                Product(
