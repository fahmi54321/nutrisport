...
data class ManageProductState(
    ...
    val thumbnail: String = "https://www.shutterstock.com/shutterstock/photos/1883859943/display_1500/stock-photo-the-word-example-is-written-on-a-magnifying-glass-on-a-yellow-background-1883859943.jpg",
        ...
        productId.takeIf { it.isNotEmpty() }?.let { id ->
            viewModelScope.launch {
                ...
                if (selectedProduct.isSuccess()) {
                    ...
                    updateCreatedAt(value = product.createdAt)
                    updateDescription(value = product.description)
                    updateThumbnail(value = product.thumbnail)
                    updateThumbnailUploaderState(value = RequestState.Success(Unit))
                    updateCategory(value = ProductCategory.valueOf(product.category))
                    updateFlavors(value = product.flavors?.joinToString(",") ?: "")
                    updateWeight(value = product.weight)
                    updatePrice(value = product.price)
                    updateNew(value = product.isNew)
                    updatePopular(value = product.isPopular)
                    updateDiscounted(value = product.isDiscounted)
                }
            }
        }
    }
        ...
        viewModelScope.launch {
            try {
                ...
                if (downloadUrl.isNullOrEmpty()) {
                    throw Exception("Failed to retrive a download URL after the upload.")
                }
